...
let main unix argv ppf ~flambda2 =
  ...
  match
    ...
    if !make_archive then begin
      ...
      | _files ->
        (* CR mvellacott: rebuild and compile the units described by the
           given .cmr files. *)
        ());
      Warnings.check_fatal ();
    end
      ...
      Compmisc.init_path ();
      let _output = Compenv.extract_output !output_name in
      (match Compenv.get_objfiles ~with_ocamlparam:false with
      | [] ->
        Compenv.fatal "Must specify at least one .cmr file with -reaper-solve"
      | _inputs ->
        (* CR mvellacott: run the reaper's whole-program solver over the
           given .cmr files, writing the solution to [_output] as a .cmsol
           file. *)
        ());
      Warnings.check_fatal ();
    end
