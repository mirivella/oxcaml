...
let reaper_lto_solve ~cmr_files ~ltosol_file =
  ...
  (* CR mvellacott: split the resulting solution into per-compilation-unit
     portions. *)
  let (solution : Flambda2_reaper.Unboxing_analysis.result) =
...
let reaped_flambda2_to_cmm ~ppf_dump:_ ~prefixname:_ ~machine_width
  ...
  let ltosol_serialisable, id_stamp_counters =
  ...
  let (_solved_dep : Flambda2_reaper.Unboxing_analysis.result) =
    Flambda2_reaper.Ltosol_format.Serialisable.deserialise ltosol_serialisable
  in
